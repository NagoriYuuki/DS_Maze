...
public:
    ...
    void InitTheme()// 初始化按钮
    {
		...
		region_back.left = (WINDOW_WIDTH) / 6 - BUTTON_WIDTH / 2 - 50;
...
int main()
{
    ...
    while (running)
    {
        ...
        {
			theme.SetThemeButtonsVisibility(false);//进入难度选择页面，隐藏主菜单按钮
        ...
        {
            cout << "游戏开始" << endl;
            theme.SetThemeButtonsVisibility(false); // 游戏开始时隐藏所有按钮
            theme.SetBackButtonVisibility(true);
            theme.BackDraw();
            if (!isCreateMaze)
            {
